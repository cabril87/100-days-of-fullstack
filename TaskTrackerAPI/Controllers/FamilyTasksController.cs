    ...
    public class FamilyTasksController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<FamilyTaskItemDTO>>> GetFamilyTasks(int familyId)
        {
            try
            {
                ...
                var tasks = await _taskSharingService.GetFamilyTasksAsync(familyId, userId);
        ...
        public async Task<ActionResult<FamilyTaskItemDTO>> AssignTaskToFamilyMember(int familyId, [FromBody] TaskAssignmentDTO assignmentDto)
        {
            try
            {
                ...
                var assignedUser = await _userRepository.GetByIdAsync(assignmentDto.AssignToUserId);
                ...
                var familyMembers = await _familyService.GetMembersAsync(familyId, userId);
                var familyMember = familyMembers.FirstOrDefault(fm => fm.User.Id == assignmentDto.AssignToUserId);
                ...
                var task = await _taskSharingService.AssignTaskToFamilyMemberAsync(
        ...
        public async Task<IActionResult> UnassignTask(int familyId, int taskId)
        {
            try
            {
                ...
                if (!success)
                {
                    return NotFound(ApiResponse<object>.NotFoundResponse($"Task with ID {taskId} not found or not assigned"));
                }
        ...
        public async Task<ActionResult<IEnumerable<FamilyTaskItemDTO>>> GetTasksForFamilyMember(int familyId, int familyMemberId)
        {
            try
            {
                ...
                // Permissions are checked in the service
                var tasks = await _taskSharingService.GetTasksAssignedToFamilyMemberAsync(familyMemberId, userId);
