...
// Add this dashboard stat rendering function
const DashboardStat = ({ 
  icon, 
  title, 
  value, 
  ...
  return (
    <Card className={`overflow-hidden border ${borderColorClass} shadow-sm hover:shadow-md transition-all`}>
      <CardHeader className={`${bgColorClass} pb-2 border-b bg-gradient-to-r from-${color}-50 to-gray-50`}>
      ...
      <CardContent className="pt-4 bg-white">
        <div className="flex items-center justify-between mb-1">
          <div className="p-2 rounded-full mr-2">
            <span className="text-3xl font-bold bg-gradient-to-r from-${color}-600 to-${color}-500 bg-clip-text text-transparent">{value}</span>
          </div>
          
          {trend && (
            <Badge variant={trend.isPositive ? "success" : "destructive"} className="ml-auto">
          ...
          <div className="mt-4">
            <div className="flex gap-1 h-10">
              {chartData.map((value, i) => (
                <div 
                  key={i}
                  className={`flex-1 bg-${color}-100 rounded-sm opacity-${Math.max(20, Math.min(100, value * 10))}`}
  ...
  return (
    <div className="container mx-auto p-4 space-y-6 bg-white min-h-screen">
      ...
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="mb-4 p-1 bg-gray-100 rounded-lg">
          ...
          <TabsTrigger value="families" className="rounded-md data-[state=active]:bg-white data-[state=active]:shadow-sm">Families</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <DashboardStat
              ...
              description={`Across ${families.length} ${families.length === 1 ? 'family' : 'families'}`}
              color="blue"
              chartData={[3, 5, 4, 6, 7, 5, 8]}
            />
            ...
            <Card className={`overflow-hidden border ${focusMode ? 'border-indigo-300 bg-indigo-50/50' : 'border-gray-200'} shadow-sm`}>
              <CardHeader className={`${focusMode ? 'bg-indigo-100' : 'bg-white'} pb-2 border-b`}>
                <CardTitle className="text-lg font-medium flex items-center gap-2">
        ...
        <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <Button variant="outline" size="sm" className="h-auto py-3 flex flex-col items-center justify-center border-primary/10" asChild>
                  <Link href="/family/create">
                    <PlusCircle className="h-5 w-5 mb-1 text-primary" />
                    <span className="text-xs">Create Family</span>
                  </Link>
                </Button>
                <Button variant="outline" size="sm" className="h-auto py-3 flex flex-col items-center justify-center border-primary/10" asChild>
                  <Link href="/family/join">
                    <LogIn className="h-5 w-5 mb-1 text-blue-500" />
                    <span className="text-xs">Join Family</span>
                  </Link>
                </Button>
                <Button variant="outline" size="sm" className="h-auto py-3 flex flex-col items-center justify-center border-primary/10" asChild>
                  <Link href="/tasks/create">
                    <FileText className="h-5 w-5 mb-1 text-amber-500" />
