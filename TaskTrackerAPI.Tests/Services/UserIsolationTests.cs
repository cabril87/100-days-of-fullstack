...
namespace TaskTrackerAPI.Tests.Services
{
    public class UserIsolationTests
    {
        ...
        public async Task CategoryService_UpdateCategoryAsync_OnlyUpdatesUserOwnedCategories()
        {
            ...
            // User 1's category
            Category user1Category = new Category { Id = 1, Name = "User 1 Category", UserId = 1 };
            
            // User 2's category
            Category user2Category = new Category { Id = 2, Name = "User 2 Category", UserId = 2 };
            ...
            // Update setup - record the call and return the updated category
            mockCategoryRepo.Setup(repo => repo.UpdateCategoryAsync(It.IsAny<Category>()))
                .Returns((Category c) => Task.FromResult(c));
            ...
            // Create update DTOs
            CategoryUpdateDTO user1CategoryUpdateDto = new CategoryUpdateDTO { Name = "Updated User 1 Category", Description = "Updated description" };
            CategoryUpdateDTO user2CategoryUpdateDto = new CategoryUpdateDTO { Name = "Updated User 2 Category", Description = "Updated description" };
            ...
            // Verify the update happened and returned correct values
            Assert.NotNull(user1UpdateResult);
            Assert.Equal(1, user1UpdateResult.Id);
            Assert.Equal(user1CategoryUpdateDto.Name, user1UpdateResult.Name);
            ...
            Assert.Equal(2, user2UpdateResult.Id);
            Assert.Equal(user2CategoryUpdateDto.Name, user2UpdateResult.Name);
            ...
            // Should return null as user doesn't own the category
